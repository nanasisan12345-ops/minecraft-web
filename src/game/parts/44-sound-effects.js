  ...
  // 溶岩が跳ねる「ポコッ」（本家の lava pop）。距離で減衰する
  function playLavaPop(x, y, z) {
    if (!actx) return;
    let gain = 0.05;
    if (Number.isFinite(x) && typeof player !== 'undefined') {
      const d = Math.hypot(x - player.pos.x, y - player.pos.y, z - player.pos.z);
      if (d > 18) return;
      gain = Math.max(0.008, 0.05 * (1 - d / 18));
    }
    const t = actx.currentTime, o = actx.createOscillator(), g = actx.createGain(), lp = actx.createBiquadFilter();
    o.type = 'triangle';
