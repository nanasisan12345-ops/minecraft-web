...
- **2026-07-12: ライトエンジン第1弾＋第2弾コア（計画書01=C4完了、02=C5の大半）を実装。`npm.cmd run check` 成功・プレビュー実測OK**。
  ...
  - **残タスク**: ① 12-lights.js の TORCH_LIGHTS(動的PointLight10本)の廃止検討（baked block光があるので視覚二重。ただしMob/動物/プレイヤーは焼き込みが無いので夜の照らし役として残す価値あり。目視比較してから判断）②手順書03(C6)=スムースライティング+AO ③手順書04(C7)=光量ベーススポーン（ワーカーのライト値をメイン側へ公開する仕組みが必要）。
