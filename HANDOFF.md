...
- **2026-07-26: 【性能】「開始直後カクカク、しばらくすると軽くなる」への対処。計測結果を必ず先に読むこと。**
  - **計測（`__mcDbg.perf()` を新設して実測）**: 描画 **3〜4ms**／天候0.31／松明ライト0.79／**その他の update は全て0.01ms前後**（動物・敵・ドロップ・落下ブロック・XP・液体シム・液体FX・RS・作物・生存）。**ゲームロジックは無実。**
  - **【重要】1回目の描画計測は 17〜18ms と出るが、これはシェーダーの初回コンパイルとテクスチャアップロード。** ウォームアップ後は 3〜4ms に落ちる。**影のON/OFF・水の法線マップのON/OFF による差はノイズレベル（2.9〜4.0ms）** ＝ 今日入れた水シェーダーは重さの原因ではない。計測するときは必ず捨て測定を1回入れること（`__mcDbg.gfx()` を2回呼ぶ）。
  - **真因はメインスレッドのジョブ予算の合計**。1フレームで ワールド生成(`WORLD_JOB_MS=3.5`) + 植生(`PLANT_JOB_MS=2.0`) + メッシュ適用(3.5) + チャンク再構築(`REBUILD_JOB_MS=2.2`) = **11.2ms** を使い、描画3.5msと合わせて約15ms＝60fpsの予算16.7msをほぼ使い切っていた。
  - **対処: ゲーム開始後だけ予算を絞った**（プリロード中はむしろ増やして、開始時点で残作業を減らす）。メッシュ適用 3.5→**2.2**（プリロード中 10→**12**）／チャンク再構築 2.2→**1.6**／ワールド生成 3.5→**2.5**（開始後のみ）／植生 2.0→**1.4**（開始後のみ）。合計 11.2→**7.7ms**。**生成は少し遅くなるがフレーム落ちが減る**というトレードオフ。
  - 実測: 移動しながらの1フレーム分のワールド処理が 中央値6.5ms→**2.6ms**、p90 **8.6ms**。
  - **残っている問題**: 最大 **28.7ms** のスパイクがある。予算チェックは「1単位の処理を終えてから」なので、**構造物ビルダー1個（村・神社・大仏など）が重いと予算を大きく超える**（`processWorldJob` の `job.buildQueue[job.bi++]()`）。これを直すには構造物生成自体を分割する必要があり、今回は見送った。**次に性能をやるならここ。**
  - **キャッシュの仕組み（把握しておくこと）**: チャンクメッシュは **IndexedDB に永続キャッシュ**されている（`writeMeshCache`/`meshDb`、34-mesh-rebuild.js）。**`MESH_WORKER_VERSION` を上げるとこれが全無効化され、次の起動が「全チャンク再ビルド」になって非常に重くなる**（今日は 25→26 で発生）。バージョンを上げるときはこの副作用を認識すること。キャッシュが効いている起動では `workerBuilds=0 / cacheHits=49` のようになる。
  - **新 `__mcDbg` フック**: **`perf(n)`（1フレーム分の処理を項目別に実測）**・**`gfx({shadow, water, pixelRatio})`（影・水の法線マップ・解像度を切り替えて描画時間を比較。1回目は捨てること）**。

- **2026-07-26: 砂・砂利の落下を実装（計画書外・ユーザー要望。本家にあって未実装だった）。メッシュ生成は無変更なので `MESH_WORKER_VERSION` 据え置き。**
  - **新ファイル `41-falling-blocks.js`**（`41-item-drops.js` の**前**に結合される。ファイル名のアルファベット順 falling < item < xp）。`FALLING_BLOCKS` 配列＋`updateFallingBlocks(dt)` を 82 のループへ。重力22・終端速度24。
  - **落ちる条件**: `isFallingType`（SAND / GRAVEL）で、真下が `supportsFallingBlock` を満たさないとき。**水・溶岩は支えにならない**ので、水面に置いた砂は沈む（本家準拠）。
  - **連鎖**: `tryFallBlockAt(x,y,z)` は真上へ `FALL_CHAIN_MAX=128` まで辿って柱ごと落とす。**同じフレームで同じセルに着地しようとするので、`landFallingBlock` は埋まっていたら上へ8マスまで積む**（これが無いと柱の一部がアイテム化して消える。実測で5個中3個しか積まれなかった）。
  - **着地**: 空気/水/溶岩なら置き換えてブロック化（**海に砂を落として埋め立てられる**。液体を塞ぐので `displaceLiquidAt` も呼ぶ）。置けなければ `ITEM_FOR_BLOCK` でアイテム化。
  - **落下跡の後始末は破壊経路と同じ**: `liquidOnBlockRemoved`（隣の海/湖から流れ込む）と `breakUnsupportedRsBlocks`（上のRS部品が剥がれる）を呼ぶ。
  - **トリガーを入れた場所**: `52-raycast.js` の `finishBreak` 末尾（真上を落とす）と通常設置の直後（宙に置いた砂が落ちる）／`39-hostile-mobs.js` の `explodeAt`（爆破跡すべて）／`82` の `__mcDbg.breakAt`。**`breakAt` に入れ忘れると実プレイでは動くのにデバッグでは再現しないので注意（実際に一度踏んだ）。**
  - **セーブ**: `SAVE.falling`（`[{x,y,z,t,vy}]`）。**落下中のまま閉じても位置と速度ごと復元される**（`loadSavedFallingBlocks` を 82 の `loadSavedDrops()` の隣で実行、`collectFallingForSave` を 51 の `collectSaveState` に追加）。旧セーブに無ければ `[]`。
  - **新 `__mcDbg` フック**: `falling()`（落下中の一覧）・`fallTest(h, t)`（足元近くの宙に h 段の柱を作って落とす）・`stepFalling(n, dt)`（rAF停止時に落下を手動で進める）。
  - 実測: 5段の柱→**5個とも落下し5個とも積み上がる（アイテム化0）**／`y=16` を掘ると真上の砂2個が落ちて `y=16,17` に積まれる／水面(y=14)に落とすと水を押しのけて砂になり埋め立てできる／落下中に死亡→保存→リロードで **`y=26.96 vy=-1.21` のまま3個復元されて落下を続行**／`npm.cmd run check` 成功・console error 0・worker errors 0 / fallbacks 0・回帰（クラフト/浸水/C9水中判定）正常。
  - **既知の制約（本家と同じもの含む）**: ①**元から宙に浮いている砂は、隣接ブロックの更新が届かないと落ちない**（本家も同じ挙動）②落下ブロックはプレイヤーにダメージを与えない（本家の窒息は未実装）③見た目は `TYPES[t].mats`（ドロップと同じ）なので**焼き込みライトが乗らない**——暗所では周囲より明るく見える。
  - **未確認: 落下の見た目**（プレビューが compositing しないため）。**次回起動時に、砂を掘って上の砂が落ちてくる様子・砂漠で掘ったときの崩れ方を目視すること。**
