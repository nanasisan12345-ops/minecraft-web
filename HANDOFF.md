...
- **2026-07-12: ライトエンジン第1弾＋第2弾コア（計画書01=C4完了、02=C5の大半）を実装。`npm.cmd run check` 成功・プレビュー実測OK**。
  ...
  - **残タスク（手順書02の仕上げ）**: ①頂点カラーを sky/block の2アトリビュートに分離し、onBeforeCompile の uniform dayFactor で「夜も松明の明るさが一定」にする（現状は焼き込みが scene 光と乗算されるため夜は松明周りも一緒に暗くなる）②その後 12-lights.js の TORCH_LIGHTS(動的PointLight10本)の廃止を検討 ③手順書03(C6)=スムースライティング+AO ④手順書04(C7)=光量ベーススポーン（ライト値をメイン側へ公開する仕組みが必要）。
