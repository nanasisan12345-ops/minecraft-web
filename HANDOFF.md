...
- **2026-07-26: 【不具合修正】水辺が階段状の板だらけになり重くなった件。`MESH_WORKER_VERSION=26`。**
  - **症状**（ユーザーのスクリーンショット）: 水面がバラバラの高さの板に分かれ、半透明が何重にも重なって見え、処理も重い。
  - **原因**: C8 の可変水面高（流水セルの上面を `(8-lv)/9` に下げる）。**自然の海/川/湖は暗黙ブロック＝満杯で描かれる**のに対し、今日の浸水機能で掘り跡へ流れ込んだ水は `lv1〜7` の流水セルになるため、**満杯の自然水と階段状の流水が混在**して水辺じゅうに段差が出る。1マス掘っただけで61セルが流水化することを実測。段差は面数も増やす（上面を常に描く＋側面が出る）ので、**半透明の重ね描画が増えて描画が重くなる**。
  - **切り分けの実測**: 液体シム1tick=**0.01ms**、液体FX走査=**0.11ms**（0.25秒ごと）→ **CPU側は無実**。描画1フレーム=**4.0ms** で、重さは描画側だった。
  - **修正**: `addLiquidBlockToState` の `topH` を **`(above === t || t === WATER) ? 1 : ...`** に変更＝**水は常に満杯で描く**。溶岩は不透明で量も少なく段差が味になるので従来どおり階段状。**34-mesh-rebuild.js と world-mesh-worker.js の両方に完全同一の式を入れること**（grep で1行に集約されることを確認済み）。
  - **失ったもの**: 水の滝の下が階段状に広がる表現（C8-1b の見た目）。本家にはある表現だが、自然水を満杯で描いている限り必ず段差が出るため見送り。**復活させるなら、自然の海/川/湖も 8/9 で描く（＝ワーカーの地形生成側で水面高を持たせる）必要がある。**
  - **併せて**: `waterSourceAt` が自然の水ブロックも源として数えるようにした（本家の無限水源。水源2つに挟まれた掘り跡は水源になる）。段差解消には効かなかったが本家準拠なので残す。氾濫は `NAT_FLOOD_CAP=1500` で頭打ち。
  - **未確認: 実機の見た目と体感の軽さ**（プレビューが compositing しないため）。**次回起動時に、水辺が平らに見えること・重さが解消したことを確認すること。** まだ重い場合の次の候補: 雨パーティクル1100本、`updateLiquidFx` の走査範囲、水シェーダーのフレネル/法線マップ。
...
- **2026-07-26: 水と溶岩の表現を本家に寄せた（計画書外・ユーザー要望「見た目と音」）。メッシュ生成には触っていないので `MESH_WORKER_VERSION` は据え置き。**
  ...
  - **`47-liquids.js` に演出セクション**: `updateLiquidFx(dt)` が **0.25秒ごとにプレイヤー周辺 15x7x15（≈1600セル）を走査**する。`liquids` Map ではなく `blockAt` を見るので**自然の海/川/湖/火山も演出の対象になる**。滝＝**「上から水が供給されていて横が開いている柱」**（`openBelow` だけで判定すると柱の先端1セルしか出ず、しぶきがほとんど出なかった）→ 柱全体から水しぶき。溶岩は露出面から 5% で火花＋`playLavaPop`、8% で煙。走査で数えた量を `liquidAmbience()` で公開。
  - **環境音（44）**: `ENV.waterGain`（バンドパス900Hz のノイズ＝せせらぎ）と `ENV.lavaGain`（ローパス190Hz＝ゴボゴボ）を追加し、`updateLiquidAudio(dt, underwater)` が近くの液体量で音量を決める（水中は0.55倍にこもらせる）。`playLavaPop(x,y,z)` は三角波150→52Hz、18mで距離減衰。会場(RAVE)中は0に落とす。
  - **新 `__mcDbg` フック**: `liquidFx()`（water/lava の量・生きている粒数・溶岩の脈動とUVオフセット）・`stepLiquidFx(n,dt)`（rAF停止時に演出を手動で回す）。
  - 実測: 溶岩の UV が 1秒で y-0.006 動き発光が 0.981→0.84 と脈動／**火山のそばで lava=60・粒27個**（火花と煙）／空中の水源から作った滝で **water=7・粒が2→4と継続発生**、`dryUp` 後は0／湖のそばで water=113（せせらぎの音量に反映）／回帰: クラフト・C9水中判定（採掘×25・霧）・浸水シム・`npm.cmd run check` 成功・console error/warn 0・worker errors 0 / fallbacks 0。
  - **未確認: 音は一切鳴らせていない**（プレビューの AudioContext が suspend のまま。ノード生成でエラーが出ていないことだけ確認）。**次回起動時に、水辺でせせらぎ・溶岩でゴボゴボとパチッ・水中でこもることを実際に聞いて確認すること。** パーティクルと溶岩の脈動の見た目も目視未確認。
  - **やっていないこと（本家との残差）**: ①流れの向きへのテクスチャスクロール（頂点UVをワーカーで回す必要あり）②水中の光の深さ減衰（ライトエンジン変更＋全チャンク再ビルド）③隣接する流水セルの高さ差の側面（C8からの既知の制約）。ユーザーと相談のうえ今回は見送り。

- **2026-07-26: 自然の水/溶岩からの浸水＝本家の「掘れば流れ込む」を実装（計画書外・ユーザー要望）。`MESH_WORKER_VERSION=25`。**
  - **前提の観測**: 洞窟生成は `h > SEA + 2` の列に限定されている（`32-world-window.js:107` / `world-mesh-worker.js:372` の `isCaveAt`）ので、**海の下には洞窟が生成されない＝「乾いた海底洞窟」は存在しない**。よってワールド生成側（海面以下の空洞を水で埋める等）の変更は不要で、実装は実行時の浸水のみ。自然の溶岩は火山の地表(`lavaCap`)・溶岩湖(`waterFeature.fill`)・構造物の中だけ。
  - **`47-liquids.js`**: `liquidOnBlockRemoved(x,y,z)`（破壊/爆発で空気になった直後に呼ぶ。`rsOnBlockChanged` と同じ立ち位置）が隣接6方向を見て、自然の水/溶岩セルを **`{t, lv:0, nat:true}` の源としてシムに取り込む**。あとは既存のフローがそのまま流す。**`nat` 源も `waterSourceAt` に数えられる（lv0 なので自動）ので、海際に掘った穴は本家どおり「水源」になる。** バケツで汲んでも `nat` 源は消えない（`pickupLiquidSource` の先頭で return）＝本家の無限水源。
  - **【重要な既存バグ修正 — これが無いと機能が成立しない】掘り跡（`edits = -1`）のセルは `blockAt` が無条件に空気を返していた**ので、そこへ流水を `setBlock` しても water として見えなかった（メイン・ワーカー両方）。修正: メインは `32-world-window.js` に `liquidCellsRef`（47が起動時に `bindLiquidCells(liquids)` で渡す）を持たせ、`edit < 0` のときだけ liquids を引く。ワーカー側は `collectMeshPayload` が **「掘り跡かつ液体セル」の edits エントリを送らない**ことで同じ結果にした（ワーカーのコードは無変更。ハッシュも変わるのでキャッシュは正しく無効化される）。**両者の blockAt の優先順位が一致していることが前提なので、片方だけ変えないこと。**
  - **セーブ**: `SAVE.natFlood`（"x,y,z" -> WATER|LAVA）に浸水の起点だけ保存。`restoreLiquids` が**「今もそこが液体なら」**復元して再シムする（地形が変わっていたらエントリを捨てる）。旧セーブに無ければ `{}`。流水セル自体は従来どおり非永続。
  - **安全弁（重くなる懸念への対策）**: `NAT_FLOOD_CAP=1500`（液体セル総数の上限。超えたら新規セルを作らず静止）。**溶岩は `LAVA_FLOOD_RATIO=0.4` で 600**。理由は実測: 火山の斜面に溶岩を流すと**落下ごとにレベルが1へ戻るため延々と流れ落ち続け、1845セルまで増えて収束しなかった**（上限導入後は600で静止し pending も0へ）。既存の `MAX_PER_TICK=400` / `PROC_CAP=40` は据え置き。**上限で止まったセルは pending から消えるので、上限を戻しても自動では再開しない（次の液体操作で再開）。**
  - **新 `__mcDbg` フック**: `floodInfo()`（cells/natural/cap/lavaCap/natSaved）・**`dryUp()`（緊急脱出。浸水と流水を全部消し `SAVE.natFlood` も空にする＝リロードで乾く）**・`floodCap(n)`（上限の一時変更）・`tp(x,y,z)`（任意座標へテレポート。遠方の海際/火山の検証用）。既存フックの不備も直した: `breakAt` が `saveEditsSoon()` を呼んでおらず掘り跡が保存されていなかった／`placeAt` が `displaceLiquidAt` を呼んでいなかった。
  - 実測（自然の湖 `-160,13,8` と火山 `-328,31,-368` で検証）: 湖底の砂を掘る→真上の自然水が nat 源になり穴が水になる（**`blockAt`=9 で水として見える＝修正の要**）／さらに2段掘ると水が下へ落ちていく／**岸の陸を水面と同じ高さで掘ると横から流れ込み、その穴が level 0 = 水源になる**（本家の無限水源）／自然水をバケツで汲んでも減らない・穴の水源は汲んでも隣の自然水から即補充される／石で塞ぐと下流が枯れる（3→1セル）／**死亡で保存→リロード→`refloodFromSave` で `natFlood` から浸水が復元**／溶岩は雪を掘った穴に lv2 で流れ込み斜面を流れ落ち **600セルで静止・pending 0**／`floodCap(2)` で上限どおり止まる／回帰: バケツ水の階段状拡散（lv 0→1→2）・クラフト・設置/破壊・ゾンビ湧き・C9の水中判定すべて正常、`npm.cmd run check` 成功・console error/warn 0・worker errors 0 / fallbacks 0（67ビルド）。
  - **未確認: 実プレイでの目視**（浸水した穴の水面の見え方、溶岩が流れ落ちる様子）。**次回起動時に、海/湖の岸を掘って水が流れ込むこと・穴を塞げば止まることを目視確認すること。**
  - **既知の制約**: ①起点はプレイヤーが壊したセルだけ（ワールド生成時から液体に面している空洞は浸水しない。上記のとおり海底洞窟は生成されないので実害なし）②上限に達すると静止したまま（本家は無制限）③溶岩が地形を焼く/引火する要素は未実装。
...
- **2026-07-26: 手順書12(C12)=クモとエンダーマン【完了】。**
  ...
  - **クモ**: 光レベル12以上（`bakedLightAt` で判定、取れなければ昼夜で代用）で `u.neutral` になりふらふら歩くだけ。壁登りは「進行先が壁なら y+3/s で上る」。
  - **エンダーマン**: カメラの視線ベクトルとの内積 >0.985 が0.5秒続くと敵対（`u.stareT`）。敵対中は3-5秒ごとにプレイヤー周囲3-7ブロックへ `enderTeleport`。水/雨で0.5秒ごとに1ダメージ＋ランダムテレポート。
  - **`damageMobBy` が `u.provoked = true` を立てる**ので、中立のクモ/エンダーマンは殴れば反撃してくる。内部ダメージ用に `damageMob(m, n)`（ノックバック無し）も追加。
  - 新アイテム: 糸 / クモの目（食料2） / エンダーパール（C17用） / 骨粉。骨1→骨粉3。**骨粉は小麦(`WHEAT_YOUNG`)を即 `WHEAT_RIPE` にし、苗木は45%で `growTree`**（48-farming.js の既存関数を流用）。スケルトンのドロップに矢0-2を追加。
  - 実測: `spawn('spider')`/`spawn('enderman')` でHP16/40のモブが出る。骨粉レシピ成立、新アイテム4種の所持を確認。console error 0。
  - **未確認: クモの壁登りとエンダーマンのテレポートの実挙動（mobのAIを手動で回すフックが無く、rAF依存のため）。骨粉の右クリックも実操作が必要。**

- **2026-07-26: 手順書11(C11)=エンチャントと金床【完了】。**
  ...
  - **金床も実装済み**: `anvilResult(a, b)` を 45 に追加。同種の道具2つ→残耐久の合計+上限の12%、道具+修理素材（`REPAIR_MATERIAL`: wood→板材/stone→丸石/iron→鉄/gold→金/diamond→ダイヤ。**アイテムIDの接頭辞で判定**）→上限の50%回復。エンチャントは高い方を引き継ぎ、付いていれば消費レベル+1（上限3）。枠は `SAVE.anvilA/anvilB`。UIは `UI.mode === 'anvil'`。フック `openAnvil(aId, aDur, bId, bDur, aEnch, bEnch)`。
  - 金床の実測: 鉄ツルハシ(上限250) 残80+残60+12%(30)=**170**・消費2／残50+鉄インゴット=**175**・消費1／効率I+耐久III と 効率III を合成→**効率III・耐久III**・耐久230・消費3／ダイヤと組み合わせ→「組み合わせ不可」。
...
- **2026-07-26: 手順書14(C14)=建材と素材の拡充【部分完了】。`MESH_WORKER_VERSION=24`。**
  ...
  - 砂利は `oreTypeAt` に1行足して地中にまばらに生成（**32-world-window.js と world-mesh-worker.js の両方に完全同一の式を入れること**。今回は差分ゼロを確認済み）。シャベル適正、硬さ0.6、**10%で火打石・90%で砂利**。これで C10 で先行追加していた火打石と打ち金が実際に作れるようになった。
  - 革は牛(1-2)と鹿(0-1)のドロップに追加。紙=小麦3（サトウキビ未実装のため手順書どおり簡略）／本=紙3+革1／本棚=板材6+本3（壊すと本3のみ、板材は返らない）。**本棚は C11 のエンチャント強化で使う。**
  - 金ツール: ITEM_DEFS に `speed: 12` を追加し、`miningTime` を `held.speed || tier表` に変更（**それまで tier だけを見ていたので、tier1の金ツールが「最速だが脆い」本家仕様にならなかった**）。耐久32。
  - **未実装（C14の残り）— 着手前に読むこと**:
    - **①羊毛**: 手順書は「既存の『布』を羊毛ブロックに改名・統合」としているが、`cloth` は**すでにセーブ内のアイテムIDとして存在**し、ベッド(`55-crafting.js`)と布の服のレシピでも使われている。改名すると既存ワールドの持ち物が消えるので、**`cloth` は残して羊毛ブロックを別途足す**のが安全。染色は「花→赤/黄染料」だが、**36-plants.js の花は装飾用のクロス描画だけでアイテム化されていない**ため、花をアイテムとして拾えるようにする作業が先に要る（ここがこのサブ機能の実質的な本体）。
    - **②氷**: 水を埋める箇所は **`32-world-window.js:125` の `if (y > h) return y <= SEA ? WATER : undefined;`** と **`world-mesh-worker.js:605` の `if (y <= SEA) return WATER + 1;`**（worker側は +1 オフセット表現）。ここで `y === SEA` かつ雪原バイオームなら ICE を返す形にする。**2つの生成器の式を完全一致させること**（砂利のときと同じ手順で diff 確認する）。加えて滑る床は `82-weather-and-loop.js:137` の `player.vel.x = mx * spd; player.vel.z = mz * spd;`（速度を即座に上書きしていて慣性が無い）を、氷の上でのみ目標速度へ補間する形に変える。**通常時の係数を1にすれば既存の操作感は完全に据え置ける。**
  - 実測: `tryRecipe` で 紙/本/本棚/金ツルハシ/金の斧/金のシャベル/金の剣/火打石と打ち金/金リンゴ が全て成立。地形走査で砂利20ブロック（同範囲で石炭271・石1693）を確認。console error 0。

- **2026-07-26: 手順書10(C10)=XPシステムと黒曜石【完了】。**
  - **新ファイル `41-xp-orbs.js`**（41-item-drops.js の直後に結合される）。`XP{level,points}` / `XP_ORBS` / `xpToNext()`（本家式 Lv0-15:2L+7 / 16-30:5L-38 / 31+:9L-158）/ `spawnXpOrb(x,y,z,value)`（7・3・1に分割）/ `spawnXpFraction()`（精錬の端数を内部蓄積して整数ぶんだけオーブ化）/ `updateXpOrbs(dt)`（吸引2.4・取得1.1、82のループから毎フレーム）。
  - **【罠】`started` は 54-input.js の `let` なので、41の時点でトップレベル参照すると TDZ で ReferenceError**（モジュール全体が死に `__mcDbg` も `__mcReady` も未定義になる。console にエラーが出ないので気づきにくい）。HUDの表示切替は `updateXpOrbs` の中で行っている。**41番台から後続ファイルの let/const をトップレベルで触らないこと。**
  - **取得源**: 討伐（39: スライム2 / その他5）／採掘（52の `ORE_XP`: 石炭1-2・ダイヤ3-7・RS鉱石1-5。**`edits.get(id) !== t` でプレイヤー設置ブロックを除外し設置→再採掘の無限XPを防ぐ**）／精錬（55の `SMELT_XP` を `st.xp` に積み、56-inventory-panel.js の `setSlot('fout')` で取り出した個数ぶんを付与＝本家と同じタイミング）／繁殖（37: 1-7）。
  - **死亡ロストは実装しない**（キープインベントリ方針）。`SAVE.xp{level,points}` は `collectXpForSave()` で保存し、無ければ0で読む。
  - 黒曜石: 硬さ45→**50**、`requiredToolTier` に **4（ダイヤツルハシ必須）** を追加。火打石(`flint`)と火打石と打ち金(`flint_and_steel`, 耐久64, 鉄1+火打石1)を ITEM_DEFS/レシピに先行追加（**火打石の入手は砂利=C14待ちで未接続**。TNT着火は既存の右クリックのままで、打ち金経由の着火は未実装）。
  - **新 `__mcDbg` フック**: `xp()` / `giveXp(n)` / `dropXp(n)` / `stepXp(n,dt)`。
  - 実測: Lv0→7でLv1、次9→Lv2で次11（2L+7）。1000付与でLv26・次92（5L-38の枝）。`dropXp(10)`→オーブ2個（7+3）が全部吸引され +10・残0。`damage(999)`で死亡してもLv26/29pt不変、リスポーン後も不変、`mc_save_*` に `xp:{level:26,points:29}` が載る。`npm run build` 成功・console error 0・既存の回帰スイート（面別UV/ドア/壁掛け松明/ベッド）全通過。
  - **未確認: 実プレイでのHUD目視（緑バーとレベル数字の見え方）、オーブの見た目。**

- **2026-07-26: 手順書13(C13)=調理と満腹度【完了】。**
  - `53-inventory.js`: 全食料に `sat`（隠し満腹度）を追加。本家準拠でリンゴ4/2.4・パン5/6.0・焼いた肉8/12.8・生肉3/1.8・ベリー2/0.4・腐肉4/0.8。**生肉は2→3、腐肉は2→4に変更**（本家値へ寄せた）。`golden_apple`（空腹4/sat9.6/吸収4）を新規追加。
  - `51-survival.js`: `SURVIVAL.saturation`（上限=現在の空腹値）と `SURVIVAL.absorb`/`absorbClock` を追加。**空腹の消耗も自然回復のコストも、まず saturation から引かれ、0になってから hunger が減る。** 被弾は `damagePlayer` 冒頭で absorb から先に削る。吸収ハートは120秒で消滅。セーブは `SAVE.player.saturation/absorb`（**旧セーブに無ければ saturation=5 / absorb=0 の既定値で読む**）。
  - `55-crafting.js`: 金リンゴ = 金インゴット8でリンゴを囲む。精錬(`SMELT_RESULT`)は `raw_meat→cooked_meat` が既存にあり、動物別の生肉は未実装なので追加なし。
  - `styles.css`: `#survivalHud .health .absorb` = 黄色ハート。
  - **新 `__mcDbg` フック**: `eat(itemId)`（食べる前後の hunger/sat/absorb/hp を返す）、`starve(n, moving)`（空腹消耗をn回早送りして遷移をログ）。**どちらも `pump(400)` → `__startGame()` の後でないと `updateSurvival` が early return するので動かない。**
  - 実測: ステーキ→hunger10→18/sat12.8、以後5tickで sat 12.8→7.8 と減り **hunger は18のまま**。ベリー→sat0.4が1tickで枯れ hunger 12→11→10。金リンゴ→満腹でも食べられ absorb=4、3ダメージで absorb 4→1・HP不変、追加3ダメージで absorb 0・HP 20→18。`npm run build` 成功・console error 0。
  - **未確認: 実プレイでのHUD目視（黄色ハートの見え方）。**

- **2026-07-26: 本家忠実化の見た目修正（計画書外・ユーザー指摘ベース）。`MESH_WORKER_VERSION=23`。**
  - **新ブロックID**: ドアのヒンジ左バリアント **118-133**（既存16ID 50-57,66-73 = ヒンジ右はセーブ互換のためID据え置き）／ベッド **134-141**（facing*2+枕元。旧1ブロックのベッド ID 40 は既存セーブのため温存し、引き続き寝られる）／壁掛け松明 **142-145**（index=松明が倒れる側 0=-z 1=+x 2=+z 3=-x）。
  - **ドア**: 32IDバリアント（facing4×開閉×ヒンジ左右×上下）。向きは**クリック面ではなくプレイヤーの視線**で決定。ヒンジは隣接ドア→クリック位置の順で決定し、`alignDoubleDoor` が設置時と開閉時に双方のヒンジを外側へ揃える（**本家は後から置いた側しか合わせないため、この対応前に置いた古いドアは1回開け閉めすれば直る**）。テクスチャは1枚扉に描き直し、厚み3/16の側面は専用の `doorEdge`。
  - **【重要な既存バグ修正】`addBoxPartToState` が6面すべてに `FACE_DEFS[0].uv` を使い回していた**ため、頂点順の違う**面1(-x)と面4(+z)でu,vが入れ替わり絵柄が90度回転**していた。面ごとの `FACE_DEFS[f].uv` を既定に変更（`part.uv` 明示時のみ全面上書き）。ドアだけでなく階段・はしご・看板・チェスト等すべてに効く。
  - **モデルの回転**: パーツに `rot: { origin:[x,y,z], axis:'x'|'y'|'z', angle }` を追加（頂点と面法線の両方を回す）。`34-mesh-rebuild.js` と `world-mesh-worker.js` の両方に同じ `applyPartRotation` を置くこと。壁掛け松明の傾き（約25度）に使用。
  - **【重要な既存バグ修正】`ITEM_DEFS` の各エントリに `id` が無く、`def.id === '...'` の設置分岐が全部死んでいた**（ベッド・ドア・フェンスゲート・苗木・小麦の種）。`53-inventory.js` の登録ループで `def.id = id` を付与して修復。
  - その他: 松明を本家準拠の 2/16角×10/16高 の1本棒に（炎はテクスチャ上部）／チェスト・作業台に専用の天板テクスチャ／手持ち表示は実テクスチャ＋実モデル形状で描き `renderOrder` を腕より上に（**手元は全部 depthTest 無しなので、同じ renderOrder だと距離順ソートで腕が道具を隠す**）／アイテムアイコンは形状シルエットで素材を抜いて重複解消（全117アイテムで重複0件）。
- **2026-07-18: 手順書17(C15)=レッドストーン基礎【完了】。鉱石/ダスト/トーチ(NOT)/レバー/石ボタン/感圧板(石・木)/ランプ＋ドア系・TNT連動（`MESH_WORKER_VERSION=17`）。ピストン/リピーター/コンパレーターは仕様どおり非実装。**
  - **新ブロックID 104-117**（22。on/offはIDバリアント方式＝回路状態は edits 経由でセーブ互換のまま復元）: RS鉱石104 / ワイヤ105 / RSトーチ106・消灯107(光レベル7) / レバー108-109 / 石ボタン110-111 / 石感圧板112-113 / 木感圧板114-115 / ランプ116-117(点灯は光レベル15)。ワイヤ=床の薄十字model・トーチ/レバー/ボタン/感圧板も専用model。テクスチャは 20 に6種追加（ボタン/板は石・板材テクスチャ流用）。
  - **新規 `47-redstone.js`**: イベント駆動の信号グラフ。`rsCells`(部品レジストリ、blockAtと自己修復同期)＋`rsWireLevel`。0.1s RSティックで「①トーチNOT更新（**前tick値で判定=本家の1tick遅延**）②ワイヤBFS再計算（源の隣=15、水平4方向+段差斜め上下に-1減衰）③出力適用」。トーチ/ワイヤが変化したら次tickを予約して伝播。**アイドル時（dirty無し・ボタン/板無し）はコスト0**。感圧板だけは存在する間0.1sポーリング（プレイヤー=縦スパン判定・モブ/動物、木の板はドロップも反応）。
  - **出力**: ドア/トラップドア/フェンスゲート=受電の立ち上がりで開・立ち下がりで閉（エッジ方式＝定常時は手動開閉と共存、受電変化が優先）。ドアは上下ペアを下セルに正規化して二重トグル防止。ランプ=定常同期（受電でON+光15、`requestEditedBlockRebuild`で広域再メッシュ）。TNT=立ち上がりで`igniteTNT`。**撤去された部品は `rsOrphans` で次tickに周囲へ断エッジを届ける**（板を壊すと隣のドアが閉じる）。
  - **ワイヤの明度**: `collectRsWireCells`→ペイロード`wireCells`→ワーカーがワイヤmodelのblockライトチャンネルを `0.10+0.90*lv/15` に（lv15=明るく発光風、lv0=暗赤）。ハッシュに含めキャッシュ整合。34フォールバックも同形。強度変化時はハロー精密判定の `markRsChunkDirty`→バッチ`rebuildChunk`。
  - **鉱石**: `oreTypeAt`（32とworkerで完全同一式）に y≤15 の帯を追加、鉄ツルハシ以上（tier3）でダスト4-5ドロップ。**採掘XPはC10のXPシステム実装時に追加**（現状XP自体が無い）。レシピ: トーチ=棒+ダスト / レバー=棒+丸石 / ボタン=石1 / 感圧板=同素材2 / ランプ=ダスト4+発光結晶の欠片1（C16でグロウストーンへ移行予定と注記済み）。
  - **設置**: `rsGround`フラグ→`placeRsGroundFromTarget`（不透明フルブロックの上面のみ）。足場を壊すと`breakUnsupportedRsBlocks`が剥がしてアイテム化。設置/破壊/爆発の全経路が`rsOnBlockChanged`でレジストリ同期。ロード時は82初回フレームの`restoreRedstone`が edits からレジストリ復元（ボタン/板の押下はOFFへ正規化→再シム）。
  - **検証（ページ内実測。rAF停止のため pump/stepRedstone 手動tick）**: レバー→ワイヤ15個が15..1で全通電しドア開、**16個目=0でランプ点かず**（受け入れ条件どおり）／レバーOFFでドア閉／ランプ受電で117+焼き込み光blk=13を実測（ライトエンジン連動）／NOT回路: レバーON→1tick遅延→トーチ消灯→隣のランプ消灯、OFFで復帰／ボタン: 押下1.0秒後に自動復帰、隣のTNTが着火（ブロック消滅）／石の感圧板に乗るとON+自動ドア開・壊すと閉／木の感圧板はドロップで反応／セーブ→リロードで回路復元（ワイヤ再計算・レバーON維持・NOT状態一致）／全シナリオで収束後 dirty=false（アイドル0）／レシピ6種一致／RS鉱石が地下に生成(30x30サンプルで10個・全てy<16)／ワイヤ明度をワーカー直起動の合成ペイロードで頂点実測(lv15→blockch=1.0, lv0→0.10)／`npm.cmd run check`成功・console error/warn 0・worker errors=0 fallbacks=0。※画面目視（ワイヤの見た目・実クリック操作）は未実施→**次回起動時に、ダストを敷いてレバーで赤が明るくなる・ランプが夜の照明になる・感圧板ドアを目視確認**。
  - **新デバッグフック**（82末尾）: `signalAt(dx,dy,dz)`/`rsStats()`/`rsPut(kind,...)`/`rsUse(...)`(レバー/ボタン)/`stepRedstone(n)`/`rsRestore()`。**汎用: `pump(n)`=rAF停止時にワールド生成/メッシュ適用ジョブを手動で回しプリロードを完了させる（→`__startGame()`でオーバーレイなしに開始できる。以後の検証がヘッドレスで可能）**。
  - **既知の制約**: ①ワイヤは全方向接続の十字見た目固定（本家の線形状適応なし）②トーチ/レバー/ボタン/感圧板は床置きのみ（壁付けなし。NOT回路は「ブロック上のトーチ」で組める）③powered block の伝導（強充電）なし=ワイヤ/源の直接隣接のみ④ボタンは石のみ（木ボタン未実装）。
- **2026-07-18: 手順書08(C8)=流れる水と溶岩【第2弾・完了】。可変水面高＋溶岩ダメージ/炎上/消火＋流れ押し＋ジュッ音を実装（`MESH_WORKER_VERSION=16`）。C8はこれで全仕様完了。**
  - **可変水面高（1b）**: シム液体セルの上面を `(8-lv)/9` に下げて描画（源lv0=8/9…lv7=1/9。自然の海/川は従来どおり満杯）。上に同液体が乗る滝の柱は満杯・上面なし。実装: 34 `collectMeshPayload` が 47 の `collectLiquidMeshCells(範囲)` で `liquidCells`([x,y,z,lv]フラット配列)をペイロード+ハッシュに追加 → worker `loadPayload` が `liquidLevels` Map 化 → `addBlockToState` の液体分岐 `addLiquidBlockToState`（上面は同液体が上に無い限り常に描画・側面/底面は従来カリング・スムースライト適用）。`addBlockFaceToState` に `topY` 引数を追加（メイン34のフォールバック経路も同形を実装）。**検証はコンソールから worker を直接起動した合成ペイロードで頂点実測**: lv0→+0.889 / lv1→+0.778 / lv4→+0.444 / lv7→+0.111、滝柱セルは上面なし側面フル、と厳密一致。
  - **溶岩仕様準拠の修正**: `LIQ_MAXLV[LAVA]` 3→6（lv0,2,4,6=平地3マス。旧値だと1マスしか流れず丸石が観測不能だった）。実測: 溶岩拡散 L0→L2,L4,L6、**流溶岩+水→丸石(39)・溶岩源+水→黒曜石(103) を両方観測**、pending→0収束。
  - **溶岩接触ダメージ・炎上（仕様4）**: 51 `updateSurvival` で溶岩接触時に既存ダメージ3+`SURVIVAL.burn=2.0`（2秒炎上）。炎上中0.8秒ごとに1ダメージ、**水に触れると即消火**。HUDのハート左に🔥表示。リスポーンで burn リセット。実測（`stepSurvival` 手動tick）: 20→17(溶岩)→溶岩撤去後も燃焼継続→16→15(炎上tick×2)→2秒で自然鎮火／再着火→水設置で即 burn=0・🔥消灯。
  - **流れ押し（仕様5）**: 47 `liquidFlowVector(x,y,z)`=レベル勾配（低lv→高lv方向）+落下口吸引の正規化xzベクトル。82 メインループでプレイヤーを `moveAxis` 経由 1.4/s で押し、41 `updateItemDrops` でドロップも同速で押し＋**水中は浮力**（vy+40/s・上限0.9で水面まで浮く）。実測: 東向き水流の全セルで flow=[1,0]・南セル[0,1]／水流に置いたドロップが1秒で+2.2m 流され y13.4→14.0 に浮上。
  - **ジュッ音**: 44 `playSizzle(x,y,z)`（highpassノイズ0.35s・26m距離減衰）。石化時に47から呼ばれる（既存の typeof ガード呼び出しが実体化）。
  - **検証環境の注意**: ペインhiddenでrAF停止のため、新フック `pump(n)`（processWorldJob/processRebuildJob/processPlantJob を手動で回す→プリロード完了→`__startGame()`）、`stepSurvival(dt,moving)`、`stepDrops(dt,n)` で全ロジックを実測した。`npm.cmd run check` 成功・console error/warn 0・worker errors=0 fallbacks=0。**画面目視（水面の階段状の見た目・実プレイでの流され感）だけ未確認 → 次回起動時に、バケツ水が階段状に見える・水流で体が流される・溶岩で🔥が出て水で消えることを目視確認。** テスト残骸: localhost開発ワールドのスポーン近くに黒曜石(-4,15,0)/丸石(-5,13,0)/石のeditが残存（SAVE.liquidsは掃除済み0件。GitHub Pages側の本番セーブは無関係）。
  - **既知の制約**: ①隣接する流水セル同士の高さ差の側面は描かない（横から見ると水面の段差に薄い隙間。本家は部分側面を描くが簡略化）②シム源(8/9)と自然水(満杯)が隣接すると水面に小さな段差③流水セルの走行はブロック格子単位（斜め流の補間なし）。
- **2026-07-13: 手順書08(C8)=流れる水と溶岩【第1弾】を実装。仕様1〜3（液体表現・シム・バケツ・セーブ）。**
  - **新規 `47-liquids.js`**: `liquids` Map（"x,y,z"→{t:WATER|LAVA, lv:0-7}, 0=源）。流水セルは `setBlock` でランタイムのみ（**永続化しない**）、源だけ `SAVE.liquids` に保存しロード時 `restoreLiquids()` で再シム。イベント駆動キュー（`pending`）方式で**アイドル時コスト0**。
  - **フローアルゴリズム**: 水tick0.25s/溶岩1.5s。落下（下が空気→lv1で落下、水平拡散せず）／水平拡散（下が塞がると lv+減衰。水-1最大7・溶岩-2最大3）。レベル＝最寄り源からのBFS距離（`newLv=min(真上供給なら1, 隣接lv+減衰)`、maxlv超で枯れる）＝不動点で収束。無限水源（シム源2つに水平隣接＋下solidで源化。※自然水は数えない＝岸辺氾濫を防止）。溶岩×水: 源+水→黒曜石(OBSIDIAN=103)/流溶岩+水→丸石/溶岩が水セルへ→石。
  - **性能の安全弁**: 二重層など病的配置で境界が2サイクル振動しうるため、`procCount`（セル毎処理回数、`PROC_CAP=40`、液体操作ごとに `clear()`）を超えたセルは凍結。**必ず pending→0 に収束**（正常な氾濫は数〜十数tickで収束、振動配置もPROC_CAP回で静止）。処理は1tick最大400セル。
  - **メッシュ**: 流水は既存の WATER/LAVA ブロックとして `setBlock`→ワーカーが通常描画（**可変水面高は未実装＝全マス満杯で描画**。見た目の「階段状」は次回1b）。液体変化のチャンク再構築は `requestEditedBlockRebuild`（毎セル rebuildPlants で重い）を避け、`dirtyLiquidChunks` に集めて tick 末に `rebuildChunk` を1回ずつ。`MESH_WORKER_VERSION` は据え置き（ブロックモデル不変）。**注意: 34-mesh-rebuild は C7 の 14 のまま。C2 で 15。C8 1a はメッシュ形状不変なので +1 不要。1b で可変高を入れるとき +1 する。**
  - **バケツ**（52）: 設置→`placeLiquidSource`（源）、汲む→源(lv0)のみ `pickupLiquidSource`（下流が枯れる）。流れ(lv>0)は汲めない。自然水源は無限のまま残す。ブロックで塞ぐと `displaceLiquidAt` で下流が枯れる。新ブロック **黒曜石(OBSIDIAN=103)**: 硬さ45・ツルハシ（ダイヤ必須化はC10予定なので今は任意ツルハシ）。
  - **検証（ページ内実測・rAF停止のため `__mcDbg.stepLiquids` で手動tick）**: 水が0→7の階段状に7ブロック拡散し停止（113セルで安定）／崖なし床でも滝は真下へ落下（[0,1,1,1,1]）／源2つ2マス間隔で中間セルが源化＝無限水源（143セルで有界・自然水近傍でも氾濫しない）／単源を汲むと下流が完全に枯れる／溶岩源+水で黒曜石(103)生成／全シナリオで pending→0（アイドルコスト0）／源のセーブ→リロードで復元・再シム／`npm.cmd run check` 成功・console error/warn 0。※画面目視（水の見た目・実バケツ操作）はスクショツール不調＋rAF停止で未実施。
  - **既知の制約/残**: ①可変水面高（見た目1b）②溶岩接触ダメージ・炎上2秒・水で消火（仕様4）③プレイヤー/アイテムの流れ押し（仕様5）④「ジュッ」音（`playSizzle` 未実装・呼び出しは typeof ガード済）⑤丸石（流溶岩+水）は同一コード経路だが本テストでは溶岩の到達距離が短く単独観測できず（黒曜石で経路は実証）。
  - **新デバッグフック**（82末尾）: `placeLiquid(kind,dx,dy,dz)`/`removeLiquid`/`liquidInfo`/`simStats`(pendingSample付)/`stepLiquids(n)`(rAF停止時の手動tick)/`refloodFromSave`。
- **2026-07-13: 手順書06(C2)=はしご・板ガラス・看板を実装**（`MESH_WORKER_VERSION=15`）。新規ブロックID: はしご94-97（4向き）/ 板ガラス98 / 看板99-102（4向き）。IDは slab(93) の次から append（C1の注意どおり既存IDの前に挿入しない）。
  - **はしご**: 薄板model（背面の壁に貼る、向き=設置面normal）。solid:false・衝突なし＝ブロック内に入って登る。設置は`placeLadderFromTarget`（52）で「不透明フルブロックの側面クリック」のみ。物理（82メインループ）: `playerOnLadder()`（50）でAABB重なりを判定し、重なっている間は重力を打ち消して **W/Spaceで上昇(2.35/s)・入力なしでゆっくり滑降(1.5/s)**、触れている間は落下ダメージを無効化。背面の壁を壊すと`breakDetachedLadders`（52 finishBreak）が剥がしてアイテム化。**※本家の「スニークで停止」は本ゲームにsneakが無い(Shift=ダッシュ)ため非採用**（W/Space=上昇・無入力=滑降に置換。操作方針の逸脱として記録）。
  - **板ガラス**: フェンス風の十字薄板（厚さ0.125・全高、collisionBoxes同形）。単一ID、通常設置（特別分岐なし）。本家同様 **壊してもドロップなし**（`blockDrops`で`[]`）。
  - **看板**: 立て看板（4向き、VILLAGE_SIGNの形状流用）。ブロック上面クリックで設置(`placeSignFromTarget`)→直後に編集ダイアログ。テキストは**チャンクメッシュと別のオーバーレイ層**（新規 `45-signs.js`）: `SAVE.signs["x,y,z"]=[4行]` を CanvasTexture→薄板plane で板前面に貼る（`refreshSignMesh`/`removeSignMesh`/`refreshAllSigns`、dispose管理）。ロード時は 82 animate 初回に `refreshAllSigns()`。設置/編集/破壊で個別に更新。右クリックで再編集（52 interactOrPlace）、破壊で`deleteSignText`がテキスト削除。**入力は input.value / Canvas fillText のみ（innerHTML不使用）**。ダイアログは releasePointerForUi/relockPointerForGame を使い、キー/クリックは stopPropagation でゲーム側に奪われない。
  - **触ったファイル**: 20-textures(はしごtex) / 22-block-types(consts+model+collision) / 53-inventory(items+ITEM_FOR_BLOCK) / 55-crafting(recipes) / 52-raycast(設置/硬さ/tool/drops/再編集/破壊) / 50-player-physics(playerOnLadder) / 82-weather-and-loop(はしご物理+看板init+__mcDbg) / 33-save-state(SAVE.signs) / 新規 45-signs.js。
  - **形状のworker対応**: モデルは blockModels ペイロード経由で worker/フォールバック両方が自動描画（追加コード不要）。看板テキストは overlay のため worker 非関与。`MESH_WORKER_VERSION 14→15`。
  - **検証（ページ内実測）**: `npm.cmd run check` 成功・console error/warn 0・mesh errors=0 fallbacks=0。全3レシピ一致（はしご3/板ガラス16/看板3）。看板設置→テキスト設定→**リロードでブロック(id 101=SIGN+2)・テキストとも復元**・overlay再生成でエラーなし。`onLadder` が はしご有無で true/false 切替。板ガラス/はしごの新モデルが worker で fallbacks=0 描画。※画面目視（テキストの見た目・実クリックでの登坂/設置操作）はスクショツール不調により未実施 → **次回起動時に、はしごを実際に登り降り(手を離すと滑降・落下ダメージ無)・板ガラスが窓に見える・看板ダイアログで日本語入力→板に表示 を目視確認**。
  - **新デバッグフック**（82末尾）: `__mcDbg.signSet(dx,dy,dz,dir,lines)`（看板+テキストを直接設置）、`signInfo()`、`ladderPut(dx,dy,dz,dir)`、`onLadder()`。
- **2026-07-13: 手順書04(C7)=光量ベースのモブスポーンを実装**（`MESH_WORKER_VERSION=14`）。敵の湧き/日光燃焼を、C4/C5で焼いた実ライトに統一。旧来の「置いた明かりの半径抑制(PLACED_LIGHTS)」「頭上ソリッド走査(isCoveredFromSky)」を全廃。
  - **ワーカー光値のメイン公開**: `src/world-mesh-worker.js` の `extractChunkLight(cx,cz)` が computeLighting 後の LGT から当チャンク16×16列の焼き込み光を1バイト/セル（`sky<<4|blk`）で取り出し、`packed.light={x0,z0,y0,y1,data}` に載せる（バッファは transfer）。`packed` はキャッシュにも乗るので **fresh/cache 両経路**で光が届く。メイン(`34-mesh-rebuild.js`)は `applyPackedChunkState` で `CHUNK_LIGHT` Map に格納、チャンク破棄/`rebuild()` で evict。公開アクセサ **`bakedLightAt(x,y,z)` → `{sky,blk}` or null**（未メッシュは null）。1チャンクの帯高 H≈95 → 抽出は約24kセルで <1ms、メモリ約24KB/チャンク。ビルド時間は従来のライトエンジン範囲（実測 avg≈180ms/初期ロード時、单発≈60ms相当、errors=0 fallbacks=0）。
  - **湧きゲート**（`39-hostile-mobs.js` `spawnLightAllows(x,y,z)`）: **blocklight=0 必須 かつ（skylight≤7 または 夜）**。データ未取得（未メッシュ）は保守的に湧かせない。`trySpawnMobs` の洞窟枝は湧きセル(床+1)、地上枝は h+1 でこのゲートを通す。地上の「昼は湧かない」早期returnを撤廃し、暗ければ日陰/屋内でも湧く本家挙動へ（※完全密閉の浅い地上部屋は heightAt が屋根を返すため surface 枝に乗らない＝この経路では湧かない。engine構造上の既存制約で回帰ではない。洞窟/地下は正しく湧く）。
  - **日光燃焼**（`updateMob`）: `isCoveredFromSky` を廃し **`bakedLightAt(mob).sky>=14 かつ 昼/朝`** で燃焼。屋根下・日陰・洞窟(sky<14)は燃えない。光データ未取得は燃やさない（保守的）。
  - **掃除**: `52-raycast.js` の `PLACED_LIGHTS`/`registerPlacedLight`/`unregisterPlacedLight`/`nearPlacedLight`/`LIGHT_BLOCK_TYPES` と編集からの復元ループ、設置(565)/破壊(405)の register/unregister 呼び出し、`explodeAt`(39) の unregister 呼び出し、`isCoveredFromSky`(39) を全削除。※12-lights.js に湧き抑制コードは無く（PLACED_LIGHTSは52にあった）、`TORCH_LIGHTS`(視覚用の動的PointLight)は HANDOFF方針どおり温存。
  - **触ったファイル**: src/world-mesh-worker.js / 34-mesh-rebuild.js / 39-hostile-mobs.js / 52-raycast.js / 82-weather-and-loop.js。
  - **検証(ページ内実測 http://localhost:5173)**: `npm.cmd run check` 成功・console error/warn 0・errors=0 fallbacks=0。松明設置で blk=13→12…8 と距離減衰し全て `canSpawnHere=false`。夜の開けた地上(blk0/sky15)=`true`、昼の開けた地上=`false`、暗所(sky0/blk0)=昼でも`true`。開けた地上に湧かせたゾンビが昼に約3秒で焼死（燃焼経路）。※画面目視は開始オーバーレイのポインタ解放でゲーム操作までは未実施 → **次回起動時に、夜に松明で囲った拠点に湧かない／洞窟で松明を並べると止まる／朝に日なたのゾンビだけ燃えることを実プレイで目視確認**。
  - **新デバッグフック**（82末尾）: `__mcDbg.lightAt(dx,dy,dz)` の戻りに `baked:{sky,blk}` を追加。`__mcDbg.spawnLightAt(dx,dy,dz)` → `{pos,baked,dayLabel,canSpawnHere}`（任意セルの湧きゲート判定）。
...
- **2026-07-12: 手順書23(C19)=岩盤・深層岩・深部洞窟の描画修正を実装**（「掘るとマップ外に落ちる」バグの根治。`MESH_WORKER_VERSION=13`）。
  ...
  - **深部洞窟の描画**: `columnYRange` の走査下限を「caveRegion > -0.26 の列は CHUNK_Y_MIN まで」に拡張（従来は海面-24=Y≈-16で打ち切られ深部洞窟の壁が描画されなかった）。**性能対策**: 深層のソリッドは `UNRESOLVED` センチネルで種類確定を遅延し、面が見えるセルだけ鉱石ノイズを解決（worker `columnStack`/`buildChunkState`）。Y=-64の下面は描かない。
  - **検証**: Nodeスモーク（scratchpad/bedrock-smoke.mjs）で7チャンク中4つが−60まで메ッシュ化・深層岩125〜425個・岩盤面2〜13・底面の無駄面0・遅延解決の前後で出力完全一致・ビルド52〜86ms（洞窟なし〜洞窟チャンク。導入前62ms）。ページ内実測でグラデーション統計・Infinity・爆発耐性を確認。console error/warn 0・errors=0 fallbacks=0。※ペインhiddenのため画面目視は未確認 → **次回起動時にF4洞窟やF8廃坑の深部で壁が欠けていないこと、真下掘りで岩盤で止まることを目視確認**。
- **2026-07-12: キープインベントリ化（ユーザー決定による本家からの意図的変更）**。死んでも持ち物・防具をドロップせず、リスポーン後・リロード後もそのまま保持する（`51-survival.js` の `diePlayer` からドロップ散布を削除）。死亡画面文言も更新。将来のXP実装（計画書10=C10）でも死亡ロストは実装しない（手順書に反映済み）。検証: give→damage(999)→死亡でINV不変・ドロップ0→リスポーンボタン→INV不変→リロード→INV不変、通常ドロップ(life=90)の生成/減衰は従来どおり、console error/warn 0。
- **2026-07-12: ライトエンジン第1弾＋第2弾コア（計画書01=C4完了、02=C5の大半）を実装。`npm.cmd run check` 成功・プレビュー実測OK**。
  - **方式**: ワーカー(`src/world-mesh-worker.js`)が skylight（列直射15＋f15差分だけ種にして横BFS減衰）と blocklight（`TYPES[].lightLevel`>0 のセルからBFS。松明14/ランタン15/提灯14/発光結晶13/溶岩15/かまど点火13）をチャンク±LIGHT_PAD(6)で解き、面ごとに頂点カラー（ブロックライトは暖色）へ焼く。輝度=0.8^(15-L)。
  - **マテリアル**: チャンク地形だけ `litChunkMats`（24-instanced-meshes.js、vertexColors=trueのクローン）。**TYPES.mats 本体は不変**（アイテムドロップ/手持ちのジオメトリに color 属性が無く真っ黒になるため触るな）。
  - **増分更新**: `requestEditedBlockRebuild(x,y,z,emitType)` に型を渡すと光源判定して半径14の広域再メッシュ（52の破壊/設置/バケツ、55のかまど点火が対応済み）。
  - **重要修正**: `collectMeshPayload` の明示ブロック収集範囲をチャンク±1→±6（ライト域）へ拡大。旧範囲ではライト域外周の設置ブロックがワーカーに見えず、密閉室に空の光が漏れた。範囲はハッシュに入るので隣接光源の変化でキャッシュも正しく無効化される。
  - **ライト域外周(±1超〜±6)の地形は「高さ場＋明示ブロック上書き」の近似**（意図的。正確な地形スタックはメッシュが使うチャンク±1のみ。境界の稀な継ぎ目は隣チャンクの自ビルドが正しく照らす）。
  - **検証済み**: 密閉石室の内部 sky=0（真っ暗）／松明 blk=14→13→12→11 の減衰（ワーカー内プローブ実測）／松明の暖色光の焼き込み／昼の地上・夜の星空は従来どおり／errors=0 fallbacks=0／console error/warn 0。
  - **性能**: 単独チャンク再ビルド実測 約63ms（初期ロードは6ワーカー並列で平均160-190ms/チャンク、ロード全体は数秒以内を維持）。`window.__mcMeshWorkerStats` に lastBuildMs/avgBuildMs/maxBuildMs を追加。
  - **新デバッグフック**（82末尾）: `placeAt(type,dx,dy,dz)`/`breakAt(dx,dy,dz)`（広域再メッシュ込みの設置/破壊）、`lightAt()`（遮蔽+最寄り光源+ビルド時間）。`window.__mcLightProbe={cells:[[x,y,z],..]}` を設定すると各ワーカービルド応答に光値が載り `window.__mcLastProbes` に溜まる。
  - **MESH_WORKER_VERSION=10**。メッシュ出力が変わる変更では必ず+1。
  - **2026-07-12 追記: 手順書02の仕上げ（sky/block 2チャンネル分離）も実装済み**。頂点属性を `mcLight`=[sky輝度, block輝度] の vec2 に変更（`MESH_WORKER_VERSION=11`）。マテリアルは 24-instanced-meshes.js の `applyChunkLightShader` が onBeforeCompile で合成: 出力 = max(シーンライト結果×sky×uMcDay, テクスチャ色×暖色(1,0.85,0.62)×block)+発光。**block チャンネルはシーン光と独立＝夜でも洞窟でも松明の明るさが一定**。共有 uniform `CHUNK_LIGHT_UNIFORM`(uMcDay, 現状1.0固定)は将来の昼夜チューニング/雨減光用の口。
  - **検証（2チャンネル化）**: Node単体テスト（scratchpadのworker-smoke.mjs方式: 密閉石箱+松明の合成ペイロード）で 松明セルblk=14→隣13→2上12・密閉内sky=0・外sky=15・lights配列=頂点×2 を確認。シェーダーはページ内でthreeを直importして注入マテリアルを1フレーム描画し、block側ピクセルが計算式どおり[204,173,126]（=0.8×暖色、シーン光非依存）・glError 0 を確認。※プレビューペインが非表示(document.hidden)でrAFが止まりゲーム画面での目視だけ未確認 → **次回起動時に洞窟の松明が夜も同じ明るさで見えることを目視確認すること**。
  - **2026-07-12 追記2: 手順書03(C6)=スムースライティング+AO も実装済み**（`MESH_WORKER_VERSION=12`）。キューブ面の4頂点それぞれで空気側セル層の4セル（自+側面2+角1）の輝度を平均し、遮蔽数からAO段階0-3（係数1.0/0.8/0.65/0.5、side1&&side2で角除外=AO3）を掛ける。対角flipは「明暗差が小さい対角で分割」方式。モデルパーツ（ドア/フェンス/松明等）は仕様どおり自セルのフラットのまま。フォールバック（メイン同期ビルド）もフラットのまま（ワーカー専用品質、fallbacks=0運用なので許容）。
  - **検証(C6)**: Nodeスモーク（worker-smoke.mjs）で 972面中604面に頂点光差（スムース有効）、501面に角の減光（AO有効）、松明直下タイルの頂点輝度が理論値0.648（=(0.8+0.64+0.64+0.512)/4）と厳密一致、probe光値は変更前と同一（光計算自体は不変）、インデックス数=面×6維持（flip正常）。性能はセルサンプルのパック整数キャッシュ化で **62.5ms**（AO導入前61.9msとほぼ同等、+70%→+1%まで圧縮）。※実画面の目視は未確認（ペインhidden）。**次回起動時に、ブロックの入隅の柔らかい陰・平地にチェッカーボード模様が出ていないことを目視確認すること**。
  - **残タスク**: ① 12-lights.js の TORCH_LIGHTS(動的PointLight10本)の廃止検討（baked block光があるので視覚二重。ただしMob/動物/プレイヤーは焼き込みが無いので夜の照らし役として残す価値あり。目視比較してから判断）②手順書04(C7)=光量ベーススポーン（ワーカーのライト値をメイン側へ公開する仕組みが必要）③手順書23(C19)=岩盤と地下描画修正。

- **2026-07-07: 建築ブロック忠実化 第1弾〜第3弾 + アイテム捨て操作を実装、`npm.cmd run check` 成功**。専用ブロック形状システムを追加し、ベッド/チェスト/開いたチェスト/松明/ランタン/提灯/看板/サボテン/小麦/苗木を薄型・小型シルエットへ修正。かまどは正面・側面・上面テクスチャを分離。木のドアは2ブロック高の上下ペアで、ブロック端の薄板+4方向+ヒンジ回転風の開閉+薄板当たり判定へ修正。木のトラップドア（右クリック開閉）、木のフェンス、木のフェンスゲート（右クリック開閉、開くと非solid）、丸石の壁も追加。`Q` で選択/ホバー中アイテムを1個、`Ctrl+Q` でスタックごと前方に投げられる。次は **階段/ハーフブロック/はしご/看板/ガラス板** から続ける。
...
- `22-block-types.js`: `TYPES[]`（描画マテリアル）と各ブロックの `const` 定数、`ITEM_FOR_BLOCK` 用。現在の最大インデックスは 73（OAK_DOOR_W_OPEN_TOP）。ブロック追加はここ。`TYPES[type].model` に箱パーツ/交差板パーツを入れると、通常立方体ではないブロック形状を描ける。solid:false transparent は水/作物/苗木と同じ扱い。フェンス/壁のように1ブロックより高い衝突は `collisionHeight`、ドアのような薄板衝突は `collisionBoxes` を使う。
...
- `41-item-drops.js`: 地面に落ちるアイテム。`spawnThrownItemDrop` と `dropSelectedItem` で `Q` / `Ctrl+Q` の本家風ドロップ操作も担当。
...
## 2026-07-07 建築ブロック忠実化 第1弾〜第3弾

- ユーザー要望: 「本家にある建築要素が不足」「既存のカマドやベッドなども本家の形を踏襲したい」「ドアは開閉ギミックもちゃんと追加」。
- **専用ブロック形状**: `22-block-types.js` の `TYPES[type].model` に `{ box:[x0,y0,z0,x1,y1,z1] }` と `{ kind:'cross', y0,y1,r }` を持てるようにし、`34-mesh-rebuild.js` と `world-mesh-worker.js` の両方で通常立方体の代わりに描画。`collectMeshPayload()` で `blockModels` を worker へ渡す。メッシュキャッシュ世代はドア/フェンス系モデル追加で `MESH_WORKER_VERSION=8`。
...
- **木のドア**: 16種類のブロックIDで4方向・開閉・上下を表現（既存互換の50-57 + 追加66-73）。`oak_door` は板材6個 `['PP','PP','PP']` で3枚。設置時に上下2ブロックの空きを確認し、クリック面/視線に応じたブロック端の薄板として置く。右クリックで上下ペアをヒンジ回転風に閉⇄開へ切替。上/下どちらを壊しても両方消え、`oak_door` 1枚をドロップ。`collisionBoxes` で開閉後の薄板位置に合わせて通行できる。
...
- **フェンス/フェンスゲート/丸石の壁**: `OAK_FENCE=60`、フェンスゲート4状態（61-64）、`COBBLESTONE_WALL=65`。フェンスと壁は支柱+横材の専用モデルで `collisionHeight:1.5`、フェンスゲートは向き(x/z)と開閉をIDで表現。閉状態は1.5ブロック高で通行を止め、開状態は `solid:false` だが `pickTarget()` がインタラクション対象として拾うので右クリックで閉め直せる。クラフトはフェンス=板材4+棒2→3、ゲート=棒4+板材2→1、壁=丸石6→6。
- **アイテム捨て操作**: 通常画面は `Q` で選択中アイテムを1個、`Ctrl+Q` でスタックごと前方へ投げる。インベントリ画面ではカーソルで持っているアイテム、またはホバー中スロットのアイテムを同じ操作で捨てる。捨てたアイテムは既存の `ITEM_DROPS` 実体として保存/吸い寄せ/拾得され、寿命は300秒。
- **検証**: `npm.cmd run check` 成功。生成物 `src/game.generated.js` は assemble で更新済み。
- **次に続ける順番**: 階段 / ハーフブロック → はしご / 看板 / ガラス板。将来的にはブロックID差分ではなく、向き・ヒンジ・上付き/下付きなどを座標メタデータとして保存すると本家により近くなる。
...
- **白紙に戻して作り直したもの**: 旧インベントリ(Map数え上げ式)・旧ホットバー(固定12ブロック)・旧クラフト(リスト式ボタン)・旧かまど(即時変換)・旧食料UI・旧宝箱(取るだけ)・旧スライム敵(無効化されていた)・旧村人取引・分散していたlocalStorageキー群。旧キー（mc_inventory_/mc_tool_durability_/mc_chests_）は読まなくなった（放置で無害）。
- **新規ファイル**: `33-save-state.js`（統合セーブ `mc_save_<seed>`、10秒オートセーブ+debounce書き込み。localStorageだが単一オブジェクトなのでIndexedDB移行が容易）、`59-progress.js`（進捗19段階、右上目標HUD+達成トースト）。`56-chest-panel.js` は削除。
- **全面書き換え**: `51-survival.js`（HP/空腹/無敵時間/飢餓/死亡画面+リスポーンボタン/ベッドリスポーン/セーブ復元 `collectSaveState()`）、`52-raycast.js`（適正ツール/硬さ/ツールレベルドロップ制限/ドロップ表/設置・インタラクション・ベッド睡眠・`PLACED_LIGHTS`）、`53-inventory.js`（`ITEM_DEFS` 一元定義+36スロット `INV`+`giveItem/takeItems/mergeIntoSlots`+アイコン生成）、`54-input.js`（数字1-9=ホットバー、E/Tab=インベントリ、会場キーはR〜P/[に一本化、DJキーは会場中のみ）、`55-crafting.js`（pattern/keys形式の形合わせレシピ+ミラー一致、かまど進行 `updateFurnaces`、チェスト状態）、`56-inventory-panel.js`（インベントリ/2x2/3x3/かまど/チェスト共通のコンテナUI。カーソル持ち、右クリ半分/1個、Shift一括、閉じたらグリッド返却）、`56-hotbar-ui.js`（9枠スロット表示+耐久バー）、`39-hostile-mobs.js`（ゾンビ/スライム/スケルトン、AI・弓矢・ノックバック・昼焼け・スポーン制御・`tryMeleeAttack`）、`57-first-person-hand.js`/`58-third-person-view.js`（選択アイテムを持つ）。
- **部分改修**: `37-animals.js`（HP/被弾/逃走/ドロップ=生肉・羊は布）、`38-travelers.js`（取引を新アイテムIDで再構築+司書/旅人の場所ヒント `hintTextFor`）、`20-textures.js`/`22-block-types.js`（COBBLESTONE=39, BED=40 追加。メッシュワーカーは typeCount/transparent をペイロードで受けるのでブロック追加は安全）、`44-sound-effects.js`（`playHurtSound`）、`82-weather-and-loop.js`（updateFurnaces/updatePlayerAttack/updateProgress/updateAutosave 配線、死亡中は移動停止、`window.__mcDbg` デバッグフック）。
- **ゲーム仕様の要点**: スタック64・道具はスロット単位耐久。鉄鉱石=石Pick以上/金・ダイヤ=鉄Pick以上でないとドロップしない。石→丸石、かまどで丸石→石・丸太→石炭も可。燃料は石炭8/丸太・板材1.5/棒0.5個ぶん。敵は夜の地上+暗い地下（プレイヤーが地下にいるとき）に湧き、`PLACED_LIGHTS`（プレイヤーが置いた松明/ランタン/提灯/発光結晶。editsから復元）の半径8で抑制、16以内湧かない/80で消滅/地上20・地下10上限/判定3秒ごと。ゾンビは昼の日なたで燃える（**プレイヤーが倒したときだけドロップ**。燃え死にで報酬が入るバグは修正済み）。ワールド生成チェストは初回オープンで `rollWorldChestLoot()` が報酬を入れ、以後は27枠の普通の収納。ベッドは夜/夕方のみ・敵14ブロック以内は不可・`SAVE.spawn` 設定+朝スキップ。
...
- 指摘「本家みたいにドラッグ&ドロップでツールベルトに置きたい」: コンテナUIに**押したままドラッグして離すと置く**方式を追加（`UI.dragFrom` を mousedown のつかみ時に記録し、invScreen の mouseup で別スロットなら `handleSlotClick` を発火。result/fout には落とせない。同じスロットで離した場合は従来のクリック式を維持）。パネルの最下段に「ツールベルト（1〜9）」ラベルも追加。
- 検証: プレビューでアイコンがスロット矩形内に収まることを getBoundingClientRect で確認、ドラッグ移動（つかむ→ツールベルトの空きで離す→配置・カーソル空）とクリック式の共存を確認。console error/warn なし。

### 2026-07-06 続き: スポーンの暗所判定を本家寄せ（ユーザー指摘）

- 指摘①「敵は夜と暗い場所だけにして」: 洞窟スポーンが「地形高さ-6より深い」だけで判定していたため、**heightAt が洞窟/峡谷のくり抜きを反映しない**ことから、峡谷の底など昼でも明るい窪地に湧けていた。`isCoveredFromSky(x,y,z)`（上へ最大48ブロック走査して solid があれば「空が見えない」）を追加し、洞窟スポーンの必須条件にした。ゾンビの日光燃焼も同判定に変更（日陰では燃えない＝本家寄せ。0.5秒ごとにだけ走査するので負荷は小さい）。
- 指摘②「会場中は敵を出さない」: これは実装済みだった（`trySpawnMobs` 冒頭の RAVE.on ガード + `updateHostileMobs` が RAVE.on 中に既存MOB/矢を全消し）。プレビューで会場ON→敵0維持→OFF復帰を確認。

### 2026-07-06 第2弾: ドロップ実体化・死亡ドロップ・弓矢・防具

- **新規 `41-item-drops.js`**: ドロップは `spawnItemDrop(x,y,z,id,n,dur,life)` で実体として出る（ブロック=ミニキューブ、素材/道具=アイコンのスプライト）。重力+着地、2.4ブロックで吸い寄せ、1.1で `giveItem`（満杯なら残る）、寿命90秒（死亡ドロップは300秒）、上限90個。`SAVE.drops` に保存されリロード後 `loadSavedDrops()`（82で呼ぶ）で復元。ブロック破壊(52)・MOB討伐(39)・動物(37)が直接giveItemせずこれを使う。チェスト/かまど破壊の中身だけは紛失防止で直接インベントリへ。
- **死亡ドロップ**: `diePlayer()` が全インベントリをその場に散らばせ、死亡画面に座標を表示。`SAVE.armor` は落とさない（装備は残る）。
- **弓/矢**: `bow`（棒3+繊維3、耐久120）を右クリックで発射、`arrow`（丸石+棒+繊維→4）を1本消費。`shootPlayerArrow()`（39）が ARROWS に `fromPlayer:true` で積み、`arrowHitTarget()` で MOB/動物に命中判定（ダメージ5+ノックバック、`damageMobBy()` 共通化）。スケルトンの矢はプレイヤーのみ判定（従来通り）。
- **防具**: `cloth_armor`(-12%)/`iron_armor`(-30%)/`diamond_armor`(-42%)、胸当て型レシピ `['M M','MMM','MMM']`。`SAVE.armor` の1スロット（インベントリ画面に装備スロット、src='armor'）。`damagePlayer()` が軽減し被弾ごとに耐久1消費、0で壊れる。空腹・奈落ダメージは軽減しない。
- **検証**: `__mcDbg` に drop/drops/tryRecipe を追加してプレビュー実測 — ドロップ生成→磁石吸引→拾得、鉄の鎧装備→10ダメージが7に軽減+耐久239、弓/矢/防具3種のレシピ一致（ミラー含む）を確認。`npm.cmd run check` 成功（43ファイル）。console error/warn なし。

- （前回まで）**コミット `e8d0734` を `main` に push 済み → GitHub Pages デプロイ成功 → ユーザーが実機で「ロードが数秒／体感スムーズ」を確認済み**（2026-07-01）。
- このコミットには、以前セッションで未コミットだった 2026-07-01 の一式（Web Worker メッシュ生成＋IndexedDB キャッシュ、ワールド上下限 `Y=-64..319` 拡張、world/airBlocks/edits のチャンク索引化）と、今回の実測ベースの高速化（下記「追記2」）をまとめて入れた。作業ツリーはこの push 時点でクリーン（生成物 `src/game.generated.js` と `.claude/` は gitignore 済み）。
- **性能の現在値（フォアグラウンド実測）**: 初期プリ生成 `pgWall≒1.5秒`、メッシュは1チャンク約37ms×コア数ぶん並列。高速飛行の最悪フレームでも `rebuild≒10ms` / `worldJob≒17ms`、約139fps、`errors=0`/`fallbacks=0`。
- **次に性能を詰めるならここ**: 残る1フレームスパイクは `worldJob` の構造物フェーズ（大型ビルダーが1フレームで丸ごと走る、≒17ms）。新規territory進入時に稀。`collectBuilders` の分割 or 大型ビルダーをフレーム分割すると消せる。頻度が低いので優先度は中。
- **計測のやり方（再掲）**: プレビュータブは `document.hidden=true` で rAF がスロットルするので、`preview_stop`→`preview_start` で開き直すとフォアグラウンドで計測できる。一時的に `window.__L` 等へ計器を仕込んで `preview_eval` で読む。Node 側の単体ベンチ（`buildChunkState` の実時間・出力一致検証）は scratchpad に置いた bench.mjs / verify.mjs 方式が有効。
- 本メモは doc のみなので単体では push しない方針（[[commit-push-style]]）。次の機能コミットに同梱する。
...
- `src/game/parts/32-world-window.js` の修正:
  ...
  - **大仏**: 緑→**青銅**化。正面シルエットを「半幅プロファイル×奥行き」で中身を詰めて造形（先細りの塔→座像へ）。大きな丸頭・伏し目・白毫・鼻・口、長い耳たぶ、張った肩、腰のくびれ、椀状の定印の手、二段の蓮華座。`addDaibutsu`(旧・小型)は未使用のまま残置、使用中は `addGiantDaibutsu`。
    - **重要（ユーザーのインゲーム指摘で修正）**: 一度「参照に無い」と思って金光背を外したが、ユーザー提示の作例（緑青の座像＋巨大な金のサンバースト後光）と並べたら一目で別物だった。→ **金の光背（後光）を復活**（背面 `zHalo=cz+7` に半径10の金円盤＋24方向の放射光）。これが「一目で大仏」の最大の記号。あわせて首をはっきりくびれさせ丸頭を独立、衣の縦襞を追加。**今後この光背を外さないこと。** 検証は front だけでなく iso も見る（実ゲームは斜め視点なので front 投影だけだと過大評価する）。
